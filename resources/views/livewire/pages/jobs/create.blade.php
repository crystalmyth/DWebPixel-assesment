<div>
    <div class="container mx-auto py-4">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-bold mb-8">Create new job posting</h1>
        </div>
        <form method="post" wire:submit.prevent="save">
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-1">
                    ...
                    <div class="border rounded shadow p-4">
                        ...
                        <div class="mb-4">
                            <label for="company_logo" class="block text-gray-700 font-medium mb-2">Company Logo</label>
                            <input type="file" wire:model="company_logo" id="company_logo" class="w-full p-2 border rounded border-gray-300">
                            @error('company_logo')<span class="text-red-500 text-xs mt-1">{{ $message }}</span>@enderror
                            @if ($company_logo)
                                <div class="mt-2">
                                    <img src="{{ $company_logo->temporaryUrl() }}" alt="Company Logo Preview" class="max-w-full h-20 rounded">
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mt-4">Save</button>
        </form>
    </div>
</div>